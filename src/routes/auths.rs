use crate::{
    app_state::AppState,
    providers::{auth::update_auth, ProviderAuth, ProviderFn as _},
};
use axum::{extract::State, http::StatusCode, response::IntoResponse};
use std::sync::{Arc, Mutex};

pub async fn update_auth_route(State(app): State<Arc<AppState>>) -> impl IntoResponse {
    match update_auth(&app).await {
        Ok(_) => (),
        Err(e) => {
